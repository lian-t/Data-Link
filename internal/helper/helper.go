...
func ReadFileByLine(file string, sort string, line int) string {
	...
	case sort == "desc":
		...
		for (-offset) <= fileSize {
			...
			lineBts = append(lineBts, char[0])
			if char[0] == '\n' {
				for i := len(lineBts) - 1; i >= 0; i-- {
					bts = append(bts, lineBts[i])
				}
				lineBts = lineBts[:0]
				line--
				if line == 0 {
...
func Int64ArrJoin(arr []int64, sep string) string {
	if len(arr) < 1 {
		return ""
	}
	var s []string
...
func Float64ArrJoin(arr []float64, sep string) string {
	if len(arr) < 1 {
		return ""
	}
	var s []string
