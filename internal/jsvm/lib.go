...
// Lib 加载内置库
// TODO:: 这个与LibInside是可以合并在一起的,
// TODO:: DocumentFind, DocumentQuery, Select 这几个函数虽然用到了 Task 对象, 但是完全没必要分开, 在 playgroud 中可以直接传 nil
func Lib(vm *otto.Otto) error {
	for _, callable := range funMap {
		if err := callable(vm); err != nil {
			return err
		}
	}
...
// LibInside 加载内部函数，主要用于检查函数
func LibInside(vm *otto.Otto) error {
	_ = vm.Set("DocumentFind", func(call otto.FunctionCall) otto.Value {
		return otto.NullValue()
	})
	_ = vm.Set("DocumentQuery", func(call otto.FunctionCall) otto.Value {
		return otto.NullValue()
	})
	_ = vm.Set("select", func(call otto.FunctionCall) otto.Value {
		return otto.NullValue()
	})
	return nil
}
