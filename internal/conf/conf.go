...
type LinkConf struct {
	...
	TaskAutoStart       bool   `toml:"task-auto-start"`
	...
	MysqldumpPath       string `toml:"mysqldump-path"`
	MongoexportPath     string `toml:"mongoexport-path"`
	LogObsEnabled       bool   `toml:"log-obs-enabled"` // 日志监控服务是否开启
	LogObsType          string `toml:"log-obs-type"`    // 日志监控服务类型:http|tcp
	LogObsHost          string `toml:"log-obs-host"`    // 日志监控服务HOST
	LogObsPort          string `toml:"log-obs-port"`    // 日志监控服务PORT
	...
	BaseDir             string // 项目目录
	WebDir              string // web目录
	RuntimeDir          string // runtime目录
	ServicePath         string // 服务临时文件地址
}
...
func LoadConf(file string) (conf *LinkConf) {
	...
	conf.RuntimeDir = fmt.Sprintf("%s%sruntime%s", conf.BaseDir, ps, ps)

	conf.LogPath = fmt.Sprintf("%slog%s", conf.RuntimeDir, ps)
	conf.TaskResumePath = fmt.Sprintf("%sresume%s", conf.RuntimeDir, ps)
	conf.TaskPath = fmt.Sprintf("%stask%s", conf.RuntimeDir, ps)
	conf.TaskErrorPath = fmt.Sprintf("%serror%s", conf.RuntimeDir, ps)
	conf.ServicePath = fmt.Sprintf("%sserver.json", conf.RuntimeDir)
...
type Resource struct {
	...
	Dsn   string                 `json:"dsn"`
	Extra map[string]interface{} `json:"extra"` // 额外值
}
