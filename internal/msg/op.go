...
const (
	...
	CmdTypeCanalStat = "CanalStat"    // canal统计事件
)
...
// DocJson 格式化json文档
func (op Op) DocJson() string {
	if op.Doc == nil {
		return ""
	}
	b, _ := json.Marshal(op.Doc)
	return string(b)
}

// DocProc 文档转换
func (op *Op) DocProc(d map[string]interface{}) {
	// 是否重新命名主键
	id, _ := d["__document_id_field__"]
	if val, ok := id.(string); ok {
		op.DocIdField = val
		op.DocIdValue, _ = d[val].(string)
		delete(d, "__document_id_field__")
	}
	// 是否重新定向操作
	opType, _ := d["__document_op_type__"]
	if val, ok := opType.(string); ok {
		switch val {
		case "insert":
			op.OptionType = OptionTypeInsert
		case "update":
			op.OptionType = OptionTypeUpdate
		case "delete":
			op.OptionType = OptionTypeDelete
		default:
		}
		delete(d, "__document_op_type__")
	}
	op.Doc = d
}
