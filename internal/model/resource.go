...
// ConnMongoDB MongoDB连接
func (m *Resource) ConnMongoDB(ctx context.Context) (*mongo.Client, error) {
	dsn := m.Dsn
	if dsn == "" {
		dsn = fmt.Sprintf("mongodb://%s:%s@%s:%s/?connect=direct", m.User, m.Pass, m.Host, m.Port)
	}
...
// ConnRabbitMQ rabbitmq链接
func (m *Resource) ConnRabbitMQ() (*amqp.Connection, error) {
	dsn := m.Dsn
	if dsn == "" {
		// 默认虚拟机 /
		dsn = fmt.Sprintf("amqp://%s:%s@%s:%s/", m.User, m.Pass, m.Host, m.Port)
	}
...
// DocumentSetMongoDB 数据集
func (m *Resource) DocumentSetMongoDB() ([]map[string]interface{}, error) {
	ctx, _ := context.WithTimeout(context.Background(), 5*time.Second)
	client, err := m.ConnMongoDB(ctx)
	if err != nil {
		return nil, err
	...
	defer client.Disconnect(ctx)
	opt := options.ListDatabasesOptions{}
	opt.SetAuthorizedDatabases(true)
	databases, err := client.ListDatabaseNames(ctx, bson.D{}, &opt)
	if err != nil {
		return nil, err
	}
	var bases []map[string]interface{}
	for _, baseName := range databases {
		switch baseName {
		case "admin", "local", "config":
			continue
		}
		collNames, err := client.Database(baseName).ListCollectionNames(ctx, bson.D{})
		if err != nil {
			continue
		}
		var collList []map[string]interface{}
		for _, collName := range collNames {
			// 只获取collection类型
			collList = append(collList, map[string]interface{}{
				"layer": "second",
				"name":  collName,
				"alias": "collection",
				"cols":  map[string]interface{}{},
