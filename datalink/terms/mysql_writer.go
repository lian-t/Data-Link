...
func (_this *MySQLWriter) Run(ch chan []*msg.Op, errC chan error, call func(int)) {
	...
	for !_this.exitF {
		...
		if len(ops) == 1 && ops[0].MessageType == msg.MessageTypeCmd {
			...
			case msg.CmdTypeMySQLDDL:
				...
				if err != nil {
					// 记录一个错误,方便查看
					call(1)
					_this.errC <- fmt.Errorf("alterTableStruct error:%s", err.Error())
				}
			}
			continue
		}
		...
		case <-time.After(timeout):
			call(len(ops))
			cancel()
			_this.errC <- errors.New("task_id:" + _this.TaskId + ", timeout write")
			for _, op := range ops {
...
func (_this *MySQLWriter) Write(ops []*msg.Op) (failed int) {
	...
	if err != nil {
		_this.errC <- errors.New("instance link error")
		return len(ops)
	}

	for _, op := range ops {
		docSet, ok := _this.DocumentSetMap[op.DocumentSetAlias]
		ns := strings.Split(docSet, ".")
		if !ok || len(ns) != 2 {
			_this.errC <- errors.New("mysql write parse database name error:" + docSet + "  error")
		...
		if docIdField == "" {
			...
			if err != nil {
				_this.errC <- errors.New("mysql write get PK error:" + err.Error())
				_this.errC <- errors.New(op.DocJson())
				failed++
				continue
			}
		...
		default:
			_this.errC <- errors.New("unknown op message OptionType")
			_this.errC <- errors.New(op.DocJson())
			failed++
			continue
		}
		...
		if err != nil {
			...
			if err != nil {
				_this.errC <- errors.AddStack(err)
				failed++
				continue
			}
			instance.UseDB(ns[0])
...
func (_this *MySQLWriter) buildUpdateFV(op *msg.Op, pkField string) (set string, where string, err error) {
	...
	if op.SourceFmt == msg.OpSourceFmtSQL {
		...
		case nil:
			// 没有主键,数据不能修改
			return "", "", errors.New("pk not exists! can't update")
		case []uint8:
			where = fmt.Sprintf("`%s`='%s' ", pkField, string(value))
		case string:
			where = fmt.Sprintf("`%s`='%s' ", pkField, value)
		case float64, float32:
			where = fmt.Sprintf("`%s`='%.6f' ", pkField, value)
...
func (_this *MySQLWriter) alterTableStruct(v interface{}) (err error) {
	...
	aliasDbName, _ := v1["mapDBName"]
	aliasTbName, _ := v1["mapTBName"]
	aliasSet := fmt.Sprintf("%s.%s", aliasDbName, aliasTbName)

	// 获取映射表
	dbName, _ := v1["dbName"]
	tbName, _ := v1["tbName"]
	srcSet := fmt.Sprintf("%s.%s", dbName, tbName)
	mapTbName, _ := _this.DocumentSetMap[aliasSet]
	if mapTbName == "" {
		err = fmt.Errorf("docSet map empty, tableName:%s", srcSet)
		return
	}
	mapSet := strings.Split(mapTbName, ".")
	if len(mapSet) != 2 {
		err = fmt.Errorf("docSet format error, tableName:%s", mapSet)
		return
	}
