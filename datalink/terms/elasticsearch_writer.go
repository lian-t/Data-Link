...
func (_this *ElasticsearchWriter) Run(ch chan []*msg.Op, errC chan error, call func(int)) {
	...
	// 遇到写入量大的时候, 自动转批量的写入.阈值需要根据具体的硬件定义(40条/s,当达到这个值时即可转为批量更新)
	bulkValue, _ := _this.TargetConf.Extra["auto_bulk"]
	bulkAuto, _ := bulkValue.(bool)
	var bulkSpeed int
	if bulkAuto {
		bulkSpeedValue, _ := _this.TargetConf.Extra["auto_bulk_speed"]
		bulkSpeedF, _ := bulkSpeedValue.(float64)
		bulkSpeed = int(bulkSpeedF)
	}
	if bulkSpeed < 1 { // 超过 30条/s 即转为批量写入
		bulkSpeed = 30
	}
	...
	for {
		...
		// 自动切换批量写入
		if bulkAuto {
			currBulkTime := time.Now()
			d := currBulkTime.Sub(prevBulkTime).Milliseconds()
			if d == 0 {
				d = 1
			}
			a := int64(len(cacheList)*1000) / d
			currBulkSpeed = int(a)
			prevBulkTime = currBulkTime
			if currBulkSpeed > bulkSpeed {
				limit = bulkSpeed + 1
			} else {
				limit = 1
			}
...
func (_this *ElasticsearchWriter) Write(ops []*msg.Op) (failed int) {
	...
	rtf = func(ctx context.Context, i int, request *http.Request, response *http.Response, err error) (time.Duration, bool, error) {
		if _this.exitF {
			return 0, false, errors.New("retrying,but cancel")
		}
		if i > 10 {
			return 0, false, err
		}
	...
	DoFunc := func() {
		...
		log.Info(fmt.Sprintf("Request ES: %d", blk.NumberOfActions()))
		resp, err := blk.Do(context.Background())
		if err != nil {
			s := _this.bodyAsString(ops)
			_this.errC <- fmt.Errorf("elasticsearch writer error, errCode:500001. reason:%s .\nbody:%s", err.Error(), s)
			failed += len(ops)
			return
		}
		if resp != nil {
			...
			for _, item := range f {
				s := _this.bodyAsString(ops)
				_this.errC <- fmt.Errorf("elasticsearch writer error errCode:500002. reason:%s \nbody:%s", item.Error.Reason, s)
			}
			failed += len(f)
		}
	}
