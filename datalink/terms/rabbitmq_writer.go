...
// clientInstance 连接实例
func (_this *RabbitMQWriter) clientInstance() (ch *amqp.Channel, err error) {
	if _this.channel != nil && !_this.channel.IsClosed() {
		return _this.channel, err
	}
	conn, err := RabbitMQConn(_this.ResourceConf)
	if err != nil {
		return nil, err
	}
	ch, err = conn.Channel()
...
func (_this *RabbitMQWriter) Run(ch chan []*msg.Op, errC chan error, call func(int)) {
	...
	// 睡眠 防止写入太快,导致rabbitMQ停止工作,单位是毫秒 1000ms=1s
	sleepValue, _ := _this.TargetConf.Extra["sleep"]
	sleepF, _ := sleepValue.(float64)
	sleep := int(sleepF)
	if sleep < 0 {
		sleep = 0
	}
