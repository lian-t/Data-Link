...
// ErrorWriter 将日志写入错误队列,用来前台显示
type ErrorWriter struct {
	WriterBaseParams
}

// NewErrorWriter es写入
func NewErrorWriter(id int, tid string, rc conf.Resource, tc conf.Writer) *ErrorWriter {
...
func (_this *ErrorWriter) Run(ch chan []*msg.Op, errC chan error, call func(int)) {
	_this.errC = errC
	_this.exitWG.Add(1)
...
// Release 释放资源
func (_this *ErrorWriter) Release() {}

// Write 设置批量刷写
func (_this *ErrorWriter) Write(ops []*msg.Op) (failed int) {
	intervalValue, _ := _this.TargetConf.Extra["interval"]
	interval, _ := intervalValue.(float64)
	for _, op := range ops {
		if interval > 0 {
			time.Sleep(time.Duration(interval) * time.Millisecond)
		}
		bytes, err := json.Marshal(op.Doc)
		if err != nil {
			_this.errC <- err
			continue
		}
